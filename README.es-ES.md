...
**[MAILBOX_SETUP.md](MAILBOX_SETUP.md) lo explica paso a paso**: qué cuenta usar,
dónde encontrar el nombre del servidor (la parte que falla siempre), cómo queda el
fichero, y una comprobación que detecta los errores habituales antes de seguir.
...
tiene que señalarlo. La única regla que se ha quedado en el texto está ahí porque
rige **tu** comportamiento y no el del agente: una contraseña pegada en un chat se
queda en esa conversación para siempre, y ningún cuidado posterior lo deshace.

Espera preguntas antes de que empiece. Si el Paso 1 ha ido bien deberían ser
pocas — y si te pide la contraseña, niégate. Eso no es un paso de estas
instrucciones.
...
- Un servicio de usuario de systemd que se ejecuta continuamente y se reinicia
  solo si falla
- Un fichero de credenciales en `~/.config/agenteiamail/env`, con permisos `600`
- Ficheros de registro y estado en `~/.local/state/agenteiamail/`
...
contenido no fiable todo el día, y cualquier cosa que lee es un posible canal de
instrucciones.**

- `roster.txt` es una lista de coincidencia exacta. Lo que no esté en ella se
  rechaza.
- El cuerpo de los correos se trata como **datos, nunca como órdenes**: un mensaje
  que le diga al agente que reenvíe algo no es una petición que el agente cumpla.
- **Añadir un destinatario a `roster.txt` es decisión tuya**, nunca respuesta a
  algo que ha llegado por correo.
...
| | |
|---|---|
| [`MAILBOX_SETUP.md`](MAILBOX_SETUP.md) | Paso 1 — el buzón y el fichero `.env` |
